...
* When you have time series data by month, and it has large data records, how will you find significant differences between this month and previous month?
	*  Contrary to a random sample of observations, in a time series setting we have a temporal dependency of the observation at time t with previous observations in the series. Therefore, we cannot apply standard t-testing to the data. We first have to deseasonalize the series. Some options include: a) work on first differences, b) remove previous year's observations (12 months lag), c) fit a polynomial model to the data and remove the time trend. Once one of these passages is applied on the data (depending on the features of the time series), we can proceed with:
